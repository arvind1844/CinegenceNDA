...
def pdf1(nda_staff):
    ...
    layout.add(Paragraph("Staff Information:",horizontal_alignment=Alignment.CENTERED, font="Helvetica-Bold"))
    ...
    layout.add(
        FlexibleColumnWidthTable(number_of_columns=2, number_of_rows=5)
        .add(Paragraph("Name:"))
        .add(Paragraph(str(nda_staff.name)))
        .add(Paragraph("Email:"))
        .add(Paragraph(str(nda_staff.email)))
        .add(Paragraph("Contact"))
        .add(Paragraph(str(nda_staff.contact)))
        .add(Paragraph("Aadhar Number:"))
        .add(Paragraph(str(nda_staff.aadhar)))
        .add(Paragraph("PAN Number:"))
        .add(Paragraph(str(nda_staff.pan_number)))
    ...
    from borb.pdf.canvas.layout.image.image import Image
    layout.add(Image(
                    "http://127.0.0.1:8000"+str(nda_staff.image.url),
                    width=Decimal(128),
                    height=Decimal(128),
                ),
    ...
    # Store
    with open("template/staff.pdf", "wb") as out_file_handle:
        PDF.dumps(out_file_handle, pdf)

    import subprocess
    subprocess.Popen(["C:\Program Files\Adobe\Acrobat DC\Acrobat\Acrobat.exe", "template/staff.pdf"])