...
def pdf3(nda_visitor):
    ...
    layout.add(
        Image(
             "http://127.0.0.1:8000"+str(nda_visitor.image.url),
            width=Decimal(128),
            height=Decimal(128),
        )
    )
    ...
    layout.add(
        ...
        .add(Paragraph("Name:"))
        .add(Paragraph(str(nda_visitor.name)))
        .add(Paragraph("Contact:"))
        .add(Paragraph(str(nda_visitor.contact)))       
    ...
    # Store
    with open("template/visitor.pdf", "wb") as out_file_handle:
        PDF.dumps(out_file_handle, pdf)

    import subprocess
    subprocess.Popen(["C:\Program Files\Adobe\Acrobat DC\Acrobat\Acrobat.exe", "template/visitor.pdf"])